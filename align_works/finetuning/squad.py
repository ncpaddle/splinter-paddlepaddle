...
def squad_convert_examples_to_features(
    ...
    del new_features
    if return_dataset == "paddle":
        # Convert to Tensors and build dataset
        all_input_ids = paddle.to_tensor([f.input_ids for f in features], dtype=paddle.int64)
        all_attention_masks = paddle.to_tensor([f.attention_mask for f in features], dtype=paddle.int64)
        all_token_type_ids = paddle.to_tensor([f.token_type_ids for f in features], dtype=paddle.int64)
        all_cls_index = paddle.to_tensor([f.cls_index for f in features], dtype=paddle.int64)
        all_p_mask = paddle.to_tensor([f.p_mask for f in features], dtype=paddle.float32)
        ...
        if not is_training:
            ...
            all_end_positions = paddle.to_tensor([f.end_position for f in features], dtype=paddle.int64)
            dataset = TensorDataset(
                [all_input_ids,
                all_attention_masks,
                all_token_type_ids,

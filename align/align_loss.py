...
# 输入fake_data
inputs = pickle.load(open('loss_input.bin', 'rb'))
...
poss_t = torch.nn.CrossEntropyLoss(ignore_index=384)
...
rl_torch.add('loss', out_torch[0].detach().numpy())
rl_paddle.add('loss', out_paddle[0].numpy())
rl_torch.add('start_logits', out_torch[1].detach().numpy())
rl_paddle.add('start_logits', out_paddle[1].numpy())
rl_torch.add('end_logits', out_torch[2].detach().numpy())
rl_paddle.add('end_logits', out_paddle[2].numpy())
